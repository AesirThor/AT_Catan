...
void setBoardToArray(int *hexOrderArray)
{
  ...
  for(int i = 0; i<NUMBER_OF_HEXES; i++)
  {
    for(int j = 0; j<NUMBER_OF_LEDS_PER_HEX; j++)
    {
      ...
      {
        case brickType:
          leds[(i*NUMBER_OF_LEDS_PER_HEX)+j] = CHSV(0, 255, ledBrightness);
        break;
        case desertType:
          leds[(i*NUMBER_OF_LEDS_PER_HEX)+j] = CHSV(0, 0, 0);
        break;
        case oreType:
          leds[(i*NUMBER_OF_LEDS_PER_HEX)+j] = CHSV(160, 160, ledBrightness);
        break;
        case sheepType:
          leds[(i*NUMBER_OF_LEDS_PER_HEX)+j] = CHSV(180, 0, ledBrightness);
        break;
        case woodType:
          leds[(i*NUMBER_OF_LEDS_PER_HEX)+j] = CHSV(100, 255, ledBrightness);
        break;
        case wheatType:
          leds[(i*NUMBER_OF_LEDS_PER_HEX)+j] = CHSV(45, 255, ledBrightness);
        break;
        default:
